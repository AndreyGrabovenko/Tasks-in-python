...
_score = cs.create_text(650, 150, text='Конфет осталось: {}'.format(
    SCORE), font=("WiGuru 2", 20), fill=rgb_to_hex((27, 36, 33)))
cs.create_text(38, 190, text='Игрок 1', font=("WiGuru 2", 24),
               fill=rgb_to_hex((35, 63, 38)), anchor=NW)
_taken_away1 = cs.create_text(38, 230, text='Конфет забрано: {}'.format(
    CHECK_PLAYER1), font=("WiGuru 2", 20), fill=rgb_to_hex((35, 63, 38)), anchor=NW)
cs.create_text(298, 270, text='Сколько забрать\nконфет?', font=(
    "WiGuru 2", 20), fill=rgb_to_hex((35, 63, 38)), anchor=NW)

_player2 = cs.create_text(991, 190, text='Игрок 2', font=(
    "WiGuru 2", 24), fill=rgb_to_hex((35, 63, 38)), anchor=NW)
_taken_away2 = cs.create_text(960, 230, text='Конфет забрано: {}'.format(
    CHECK_PLAYER2), font=("WiGuru 2", 20), fill=rgb_to_hex((35, 63, 38)), anchor=NW)
cs.create_text(710, 270, text='Сколько забрать\nконфет?', font=(
    "WiGuru 2", 20), fill=rgb_to_hex((35, 63, 38)), anchor=NW)

while True:
    def update():
        global _taken_away2, _taken_away1, _score, _player2
        global _won1, _won2, _won3, count1, count2, count3, CHECK_PLAYER1
        global CHECK_PLAYER2, CHECK_PLAYER3, SCORE, temp1, temp2, temp3
        CHECK_PLAYER1 = 0
        CHECK_PLAYER2 = 0
        CHECK_PLAYER3 = 0
        SCORE = 28
    ...
    button_player1_3 = Button(root, text='Забрать', font=(
        "WiGuru 2", 19), bg=rgb_to_hex((41, 50, 60)), command=take1)
    ...
    button_player2_3 = Button(root, text='Забрать', font=(
        "WiGuru 2", 19), bg=rgb_to_hex((41, 50, 60)), command=take2)
    button_player2_3.place(x=823, y=410)

    button_player = Button(root, text='Играть с компьютером?', font=(
        "WiGuru 2", 19), bg=rgb_to_hex((151, 149, 240)), command=computer)
    button_player.place(x=470, y=540)
